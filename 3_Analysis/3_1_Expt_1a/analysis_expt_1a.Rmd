...
source("../3_1_Expt_1a/R_rainclouds.R")
```

# 读取目录下数据 Expt_1a
```{r}
# 设置目标文件夹相对路径
folder_path <- "../../2_Experiments/2_1_Pilot_Experiment/2_1_2_RawData/RawData"
...
```{r}
# 创建输出文件夹（如果不存在）
output_path <- "../../2_Experiments/2_1_Pilot_Experiment/2_1_3_CleanData/Expt_1a"
...
for (file_name in names(df_list_1)) {
  ...
  # 提取分割后的第二个元素（假设数字部分在第二个位置，可根据实际调整）并转换为数值
  participant_data$Participant_ID <- as.numeric(lapply(split_data, function(x) substr(x[[2]], 1, 2)))
...
```{r}
# 设置目标文件夹相对路径
folder_path <- "../../2_Experiments/2_1_Pilot_Experiment/2_1_3_CleanData/Expt_1a"
...
data_color_all <- data %>%
  filter(
    Participant_ID %in% c( seq(11, 25)),
    rt>=200 & rt <= 3000)#筛出反应时在200~3000)

data_color_secd <- data %>%
  filter(
    Participant_ID %in% c( seq(18, 25)),
    rt>=200 & rt <= 3000)#筛出反应时在200~3000)
```
...
matching_task_analysis <- function(data){
  ...
  # 计算所需要的画图数据1
  matching_task_data1 <- data %>% #选择正式实验的数据
    filter(part == "match_RDK") %>%
    group_by(Participant_ID, match) %>%  #按被试与匹配条件分组
    summarise(
  ...
  print(matching_task_data1)

  # ACC plot (match condition)
  acc_plot <- ggplot(
    matching_task_data1,
    aes(x = match, y = acc, fill = match , colour = match)) +
    geom_flat_violin(
      position = position_nudge(x = .2, y = 0), adjust = 1.2, trim = FALSE, alpha = 0.5) +
    geom_jitter(aes(color=match), width = 0.1) +
    # geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.5) +
    geom_boxplot(aes(x = match, y = acc), outlier.shape = NA, alpha = 0.5, width = .1, colour = "BLACK") +
    ...
    ggtitle("Matching type")

 # RT plot (match condition)
  rt_plot <- ggplot(
    matching_task_data1,
    aes(x = match, y = avg_rt, fill = match, colour = match)) +
    geom_flat_violin(
      position = position_nudge(x = .2, y = 0), adjust = 1.2, trim = FALSE, alpha = 0.5) +
    geom_jitter(aes(color=match), width = 0.1) +
    # geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.5) +
    geom_boxplot(
      aes(x = match, y = avg_rt), outlier.shape = NA, alpha = 0.5, width = .1, colour = "BLACK") +
  ...
  # ggsave("fig2_matching_task.png", final_plot, width = 10, height = 8, dpi = 300)
   ...
   # 计算所需要的画图数据2
  matching_task_data2 <- data %>% #选择正式实验的数据
    filter(part == "match_RDK") %>%
    group_by(Participant_ID, association) %>%  #按被试与匹配条件分组
    summarise(
      avg_rt = mean(rt, na.rm = TRUE),#每个被试所有trial的平均反应时
      max_rt = max(rt, na.rm = TRUE),#每个被试所有trial的最大反应时
      min_rt = min(rt, na.rm = TRUE),#每个被试所有trial的最小反应时
      sd_rt = sd(rt, na.rm = TRUE), #每个被试所有trial的反应时的方差
      all_count = n(),#每个被试在每个条件的总trial数量
      row_count = sum(rt >= 200 & rt <= 2000, na.rm = TRUE), #舍弃按键太快和按键太慢的反应时
      correct_count = sum(correct == 1 & rt>=200 & rt <=3000, na.rm = TRUE),
      acc = correct_count /all_count,
      .groups = "drop")  #计算每个被试在每个条件的正确率= 正确/总数

  # ACC plot2 (association condition)
  acc_plot2 <- ggplot(
    matching_task_data2, 
    aes(x = association, y = acc, fill = association , colour = association)) +
    geom_flat_violin(position = position_nudge(x = .2, y = 0), adjust = 1.2, trim = FALSE, alpha = 0.5) +
    geom_jitter(aes(color = association), width = 0.1) +
    # geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.5) +
    geom_boxplot(aes(x = association, y = acc), outlier.shape = NA, alpha = 0.5, width = .1, colour = "BLACK") +
    ylab('ACC') + xlab(' ') + coord_flip() +
    theme_cowplot() + guides(fill = "none", colour = "none") +
    scale_colour_manual(values = colors) + # palette = "Dark2"
    scale_fill_manual(values = colors) + # palette = "Dark2"
 ...
 # RT plot2 (association type)
  rt_plot2 <- ggplot(
    matching_task_data2,
    aes(x = association, y = avg_rt, fill = association, colour = association)) +
    geom_flat_violin(
      position = position_nudge(x = .2, y = 0), adjust = 1.2, trim = FALSE, alpha = 0.5) +
    geom_jitter(aes(color = association), width = 0.1) +
    # geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.5) +
    geom_boxplot(aes(x = association, y = avg_rt), outlier.shape = NA, alpha = 0.5, width = .1, colour = "BLACK") +
    ylab("RT (ms)") + xlab(' ') + coord_flip() +
    theme_cowplot() + guides(fill = "none", colour = "none") +
    scale_colour_manual(values = colors) + # palette = "Dark2"
    scale_fill_manual(values = colors) # palette = "Dark2"
  ...
  #输出保存到文件夹
  output_file_name <- paste0("matching_task_", deparse(substitute(data)), ".png")
...
### matching task
```{r 正确率和dprime}
matching_task_analysis1.1 <- function(data) {
...
matching_task_analysis1.1(data_motion)
matching_task_analysis1.1(data_color_secd)
```
...
```{r 正确率和dprime}
matching_task_analysis1.2 <- function(data) {
  ...
  target_folder_path <- "../3_1_Expt_1a/output"
   ...
   sdt_all <- data %>% #选择正式实验的数据
    ...
    sdt_all$betweenVari <- ifelse(sdt_all$Participant_ID < 11, "motion", "color")
    # 设置为环境变量    
...
matching_task_analysis1.2(data_all)
```   
...
matching_task_analysis2 <- function(data){
  ...
  matching_task_data1 <- data %>% #选择正式实验的数据
    filter(part == "RDK") %>%
    group_by(Participant_ID, difficulty) %>%  #按被试与匹配条件分组
    summarise(
      avg_rt = mean(rt, na.rm = TRUE),#每个被试所有trial的平均反应时
      max_rt = max(rt, na.rm = TRUE),#每个被试所有trial的最大反应时
      min_rt = min(rt, na.rm = TRUE),#每个被试所有trial的最小反应时
  ...
  acc_plot <- ggplot(
    ...
    geom_flat_violin(
      position = position_nudge(x = .2, y = 0), adjust = 1.2, trim = FALSE, alpha = 0.5) +
    geom_jitter(aes(color= difficulty), width = 0.1) +
    # geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.5) +
