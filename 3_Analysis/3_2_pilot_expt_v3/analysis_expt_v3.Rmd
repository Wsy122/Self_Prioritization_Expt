  ...
  filter(
    ...
    mutate(
      difficulty = case_when(
        step == "pract" & row_number() <= 40 ~ "easy_pract",
        step == "pract" & row_number() > 40  ~ "hard_pract",
        step == "formal" & row_number() <= 200 ~ "easy_formal",
        step == "formal" & row_number() > 200 ~ "hard_formal",
        TRUE ~ "other"
      )
    ) %>%
  ungroup()
...
ggscatterhist(
  df_motion, x ='coherence', y = 'acc', shape=21,fill="#00AFBB",color = "black",size = 3, alpha = 1,
...
```
```{r}
xname='coherence'  ##指明X轴的变量
##构建模型
fml <- "acc~s(coherence,bs='cr')"
fit<-gamm(formula(fml),random=list(Participant_ID=~1),data=df_motion,family=gaussian)
temp_ <- gamm_data(fit=fit,xname='coherence',group=NA,dt=dt)
```
