...
```{r}
# 设置目标文件夹相对路径
folder_path <- "../../2_Experiments/2_1_Pilot_Experiment/2_1_2_RawData/pilot_expt_v4.3"
...
```{r}
# 创建输出文件夹（如果不存在）
output_path <- "../../2_Experiments/2_1_Pilot_Experiment/2_1_3_CleanData/pilot_expt_v4.3"
...
```{r}
# 设置目标文件夹相对路径
folder_path <- "../../2_Experiments/2_1_Pilot_Experiment/2_1_3_CleanData/pilot_expt_v4.3"
...
```{r}
copmute_acc_onlyDiff <- function(data){
  data <- data %>%
...
copmute_acc <- function(data){
  data <- data %>%
  filter(part %in% c("match_RDK","RDK")) %>%
    group_by(subj_id, part, difficulty, association) %>%  #按被试与难度和关联类型分组
    summarise(
      avg_rt = mean(rt, na.rm = TRUE),#每个被试所有trial的平均反应时
      max_rt = max(rt, na.rm = TRUE),#每个被试所有trial的最大反应时
      min_rt = min(rt, na.rm = TRUE),#每个被试所有trial的最小反应时
...
matching_task_analysis <- function(data){
 ...
 #  matching_task_data <- matching_task_data[matching_task_data$acc >= lower_bound_motion & matching_task_data$acc <= upper_bound_motion, ]
  ...
  acc_plot <- ggplot(
    ...
    geom_jitter(aes(x = difficulty, y = acc), width = 0.15) +
...
matching_task_analysis(data_motion_onlyDiff)
...
```{r}
# 调用函数
rdk_task_analysis(data_motion_onlyDiff)
