...
# 确保 rt 和 correct 列是数值型
data <- data %>%
  mutate(rt = as.numeric(rt), 
         correct = as.numeric(correct)) %>%
  filter(!is.na(rt), !is.na(correct))

# 计算 match_RDK 的平均 RT 和标准差
summary_data_match <- data %>%
...
# 绘制 match_RDK 的条形图
p1 <- ggplot(summary_data_match, aes(x = association, y = mean_rt, fill = association)) +
  ...
  coord_cartesian(ylim = c(0, max(summary_data_match$mean_rt + summary_data_match$sd_rt, na.rm = TRUE))) +
  cbp +
  theme_minimal() +
...
# 绘制 RDK 的条形图
p2 <- ggplot(summary_data_RDK, aes(x = association, y = mean_correct, fill = association)) +
  geom_col(color = "black", width = 0.35, position = position_dodge(width = 0.4)) +
  geom_errorbar(aes(ymin = mean_correct - sd_correct, ymax = mean_correct + sd_correct), 
                width = 0.2, position = position_dodge(width = 0.4)) +
  labs(x = "Identity", y = "Accuracy") +
...
# 合并两个图形
combined_plot <- plot_grid(p1, p2, align = "hv", ncol = 2)
print(combined_plot)
