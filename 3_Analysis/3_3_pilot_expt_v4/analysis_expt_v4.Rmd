...
```{r}
# 设置目标文件夹相对路径
folder_path <- "../../2_Experiments/2_1_Pilot_Experiment/2_1_2_RawData/pilot_expt_v4.1"

# 列出所有含有"pilot_expt_v4_"的csv文件
file_list <- list.files(
  path = folder_path,
  pattern = "*pilot_expt_v4-1_.*\\.csv$", full.names = TRUE
)
...
data_motion <- data %>%
  filter(
    subj_id %in% c( seq(1, 10)),
    rt >= 200 & rt <= 3000)#筛出反应时在200~3000)

data_motion <- copmute_acc(data_motion)
...
data_color <- data %>%
  filter(
    subj_id %in% c( seq(11, 20)),
    rt>=200 & rt <= 3000)#筛出反应时在200~3000)

data_color <- copmute_acc(data_color)
...
```{r}
colors <- c(c("lightblue", "orange", "grey", "green"), c("purple", "black","brown"))
...
matching_task_analysis <- function(data){
 ...
 #  matching_task_data <- matching_task_data[matching_task_data$acc >= lower_bound_motion & matching_task_data$acc <= upper_bound_motion, ]
  ...
  acc_plot <- ggplot(
    ...
    geom_jitter(aes(x = difficulty, y = acc, color= subj_id), width = 0.15) +
    # geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.5) +
    geom_boxplot(
      aes(x = difficulty, y = acc), outlier.shape = NA, alpha = 0.7, width = .1, colour = "BLACK") +
 ...
 # RT plot (match condition)
  rt_plot <- ggplot(
    ...
    geom_flat_violin(
      position = position_nudge(x = .2, y = 0), adjust = 1.2, trim = FALSE, alpha = 0.8) +
    geom_jitter(aes(color= subj_id), width = 0.1) +
    # geom_point(position = position_jitter(width = .15), size = .25, alpha = 0.5) +
    geom_boxplot(
      aes(x = difficulty, y = avg_rt), outlier.shape = NA, alpha = 0.7, width = .1, colour = "BLACK") +
